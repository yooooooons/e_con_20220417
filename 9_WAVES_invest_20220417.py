...
#몇 일 기준 이동평균선을 구할것인가
mv_avg_duration_1 = 10
mv_avg_duration_2 = mv_avg_duration_1 - 0

# close 가격 변동 비율에 따른 매수 / 매도 결정
mv_avg_ratio_buy_cri = 1.0028

#close_ratio_sell_cri = 0.9999
mv_avg_ratio_sell_cri = 0.99995

series_down = 30    # 몇개 이상의 시간대에서 moving_average가 하락세를 유지하는가  (13)
acceptable_rise = 1.00001   # series_down count중에, 최소값대비 어느정도 상승까지는 연속된 하락으로 인정하는가   (1.0008)

sell_ratio = [0.964, 1.049]

# 거래량이 평상시의 몇 배 이상일시 매수
volume_buy = 0.15
volume_sell = 0.15

# 매도시 참조할 mv_avg값(mv_avg_duration_1, mv_avg_duration_2)의 각각의 참조 가중치
weight_avg_1 = 1   # mv_avg_duration_1 참조 비율
weight_avg_2 = (1 - weight_avg_1)   # mv_avg_duration_2 참조 비율
...
while True:
    ...
    if (now.minute % time_unit == 0) & (55 < (now.second % 60) <= 59) :   # N시:00:02초 ~ N시:00:07초 사이 시각이면
        ...
        if bought_state == 1 :   # 매수가 되어 있는 상태라면
            ...
            DF_mov_avg_check_SELL['ratio_mv_avg_1_2_avg'] = (weight_avg_1 * DF_mov_avg_check_SELL['ratio_mv_avg_1']) + (weight_avg_2 * DF_mov_avg_check_SELL['ratio_mv_avg_2'])
